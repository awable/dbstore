...
class Data(object):
    ...
    @classmethod
    def add(cls, gid1, gid2, attrs={}, get=False):
    ...
    def __setattr__(self, attr_name, attr_value):
        ...
        attr_def = self.__attrs__.get(attr_name)
        if attr_def:
    ...
    def _save(self):
        ...
        attrs = {}
        for attrname, attrdef in self.__attrs__.iteritems():
            attrvalue = getattr(self, attrname)
            assert not (attrdef.required and attrvalue is None), \
                "attr {} is required".format(attrname)
            if attrvalue is not None:
                attrs[attrname] = attrdef._to_base_type(attrvalue)

        data = Data._encoders[Data._currentEncodingIndex].encode(attrs)
        ...
        edgedata = EDGESTORE.add(self.__edgetype__, gid1, gid2, encoding, data, [], overwrite)
        cls.lastAddWasOverwrite = EDGESTORE.lastAddWasOverwrite

        # set the updated revision
        edgetype, gid1, gid2, revision, encoding, data = edgedata
    ...
    def _generateIndices(self):
        return [index.tuple(self.__datadict__) for index in self.__index__]

    def _delete(self):
        return EDGESTORE.delete(self.__edgetype__, self.__gid1__, self.__gid2__)

    def __new__(cls, gid1, gid2, attrs=None):
        ...
        if not instance:
            instance = super(Data, cls).__new__(cls, gid1, gid2, attrs)
    ...
    def __init__(self, gid1, gid2, attrs=None):
    ...
    def debug_print(self):
        ...
        padding = max(len(attrname) for attrname in self.__attrs__)
        ...
        for attrname in self.__attrs__:
